...
class CatDropBot:
    ...
    def generate_boc(self) -> str:
        """Генерация тестового BOC с случайным числом"""
        random_num = random.randint(100000, 1000000)
        boc_value = f"test{random_num}"
        return boc_value
    ...
    async def confirm_purchase(self, session: ClientSession, intent_id: str, boc: str) -> Optional[Dict[str, Any]]:
        """Подтверждение покупки с BOC от кошелька"""
    ...
    async def run_forever(self):
        ...
        print(f"{YELLOW}🔧 Режим BOC: test + random(100000-1000000){RESET}")
        ...
        async with ClientSession(timeout=timeout) as session:
            ...
            while True:
                ...
                # Генерация BOC с случайным числом
                boc_value = self.generate_boc()
                print(f"{CYAN}   ├─ Сгенерирован BOC: {boc_value}{RESET}")
                
                # Шаг 2: Подтверждение покупки
                confirm_result = await self.confirm_purchase(session, intent_id, boc_value)
    ...
    async def run_once(self):
        ...
        async with ClientSession(timeout=timeout) as session:
            ...
            if intent_data:
                ...
                boc_value = self.generate_boc()
                print(f"{CYAN}Сгенерирован BOC: {boc_value}{RESET}")
                await self.confirm_purchase(session, intent_id, boc_value)
